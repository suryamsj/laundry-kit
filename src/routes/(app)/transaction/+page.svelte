...
<Table striped={true} hoverable={true} shadow>
    <TableHead>
        ...
        <TableHeadCell>Berat (Kg)</TableHeadCell>
    ...
    <TableBody class="divide-y">
        {#each transactions as transaction, i}
            <TableBodyRow>
                ...
                <TableBodyCell
                    >{formatDateToDDMMYYYY(
                        new Date(parseInt(transaction.trans_date))
                    )}</TableBodyCell
                >
                <TableBodyCell>{transaction.customer_name}</TableBodyCell>
                <TableBodyCell>{transaction.weight} Kg</TableBodyCell>
                <TableBodyCell
                    >{formatDateReverse(transaction.finish_date)}</TableBodyCell
                >
                <TableBodyCell>{transaction.price}</TableBodyCell>
                <TableBodyCell>
                    {#if transaction.status === "proses"}
                        <Badge color="yellow">{transaction.status}</Badge>
                    {:else if transaction.status === "cuci"}
                        <Badge color="indigo">{transaction.status}</Badge>
                    {:else}
                ...
                <TableBodyCell>
                    <div class="grid lg:gap-1 xl:gap-1 lg:grid-cols-3">
                        <Button
                            href="/transaction/detail/{transaction.id_t}"
                            color="blue"
                            ><svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="16"
                        ...
                        >
                        <Button
                            href="/transaction/{transaction.id_t}"
                            color="blue"
                            ><svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="16"
                        ...
                        <form
                            bind:this={form}
                            action="?/delete&id={transaction.id_t}"
                            method="post"
                            use:enhance
                        >
                            <Button
